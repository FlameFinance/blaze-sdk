...
let PAIR_ADDRESS_CACHEv2: { [token0Address: string]: { [token1Address: string]: string } } = {}
...
export class Pairv2 {
  ...
  public static getAddress(tokenA: Token, tokenB: Token): string {
    ...
    if (PAIR_ADDRESS_CACHEv2?.[tokens[0].address]?.[tokens[1].address] === undefined) {
      PAIR_ADDRESS_CACHEv2 = {
        ...PAIR_ADDRESS_CACHEv2,
        [tokens[0].address]: {
          ...PAIR_ADDRESS_CACHEv2?.[tokens[0].address],
          [tokens[1].address]: getCreate2Address(
            FACTORY_ADDRESSv2,
            keccak256(['bytes'], [pack(['address', 'address'], [tokens[0].address, tokens[1].address])]),
            INIT_CODE_HASHv2
  ...
  public getOutputAmount(inputAmount: TokenAmount): [TokenAmount, Pairv2] {
    invariant(this.involvesToken(inputAmount.token), 'TOKEN')
    if (JSBI.equal(this.reserve0.raw, ZERO) || JSBI.equal(this.reserve1.raw, ZERO)) {
      throw new InsufficientReservesError()
    }
  ...
  public getInputAmount(outputAmount: TokenAmount): [TokenAmount, Pairv2] {
    invariant(this.involvesToken(outputAmount.token), 'TOKEN')
    if (
      JSBI.equal(this.reserve0.raw, ZERO) ||
      JSBI.equal(this.reserve1.raw, ZERO) ||
